...
namespace Database {
    public class ResultDB : SqliteHelper {
        private const String Tag = "ResultsDb:\t";
  ...
  */
        ...
        public ResultDB() : base() {
            ...
            dbcmd.CommandText = "CREATE TABLE IF NOT EXISTS " + TABLE_NAME + " ( " +
                ...
                "FOREIGN KEY(" + KEY_STUDENT + ") REFERENCES Students(id) ON DELETE CASCADE, " +
                "FOREIGN KEY(" + KEY_OBJECT + ") REFERENCES Objects(id) ON DELETE CASCADE); " +
                "CREATE UNIQUE INDEX IF NOT EXISTS idx_results_student_object ON " + TABLE_NAME + " (" + KEY_STUDENT + ", " + KEY_OBJECT + ");";
...
VALUES (1, 
        ...
        */
        ...
        public void addCorrectResult(StudentEntity studentEntity, ObjectEntity objectEntity) {
            IDbCommand dbcmd = getDbCommand();
            dbcmd.CommandText =
                "INSERT OR REPLACE INTO " + TABLE_NAME
                + " ( "
                ...
                + studentEntity._id + "', '"
                + objectEntity._id + "', "
                + "(SELECT COALESCE(SUM(" + KEY_CORRECT + "),0) FROM " + TABLE_NAME + " WHERE " + KEY_STUDENT + " = " + studentEntity._id + " AND " + KEY_OBJECT + " = '" + objectEntity._id + "') + 1, "
                + "(SELECT " + KEY_INCORRECT + " FROM " + TABLE_NAME + " WHERE " + KEY_STUDENT + " = " + studentEntity._id + " AND " + KEY_OBJECT + " = '" + objectEntity._id + "')"
                + " )";
            dbcmd.ExecuteNonQuery();
        }
...
VALUES (1, 
        ...
        */
        ...
        public void addIncorrectResult(StudentEntity studentEntity, ObjectEntity objectEntity) {
            ...
            dbcmd.CommandText =
                ...
                + studentEntity._id + "', '"
                + objectEntity._id + "', "
                + "(SELECT " + KEY_CORRECT + " FROM " + TABLE_NAME + " WHERE " + KEY_STUDENT + " = " + studentEntity._id + " AND " + KEY_OBJECT + " = '" + objectEntity._id + "'),"
                + "(SELECT COALESCE(SUM(" + KEY_INCORRECT + "),0) FROM " + TABLE_NAME + " WHERE " + KEY_STUDENT + " = " + studentEntity._id + " AND " + KEY_OBJECT + " = '" + objectEntity._id + "') + 1);";
            dbcmd.ExecuteNonQuery();
        }
...
FROM Results; 
        */
        ...
        public IDataReader getDataFromResults(int _idStudent, string _idObject) {
            ...
            dbcmd.CommandText =
                ...
                + "(SELECT COALESCE(SUM(" + KEY_INCORRECT + "), 0) FROM " + TABLE_NAME + " WHERE " + KEY_STUDENT + " = " + _idStudent + " AND id_object = '" + _idObject + "') as 'Errores usuario',"
                    + "(SELECT COALESCE(SUM(" + KEY_CORRECT + "), 0) FROM " + TABLE_NAME + " WHERE " + KEY_OBJECT + " = '" + _idObject + "') as 'Aciertos grupo', "
                    + "(SELECT COALESCE(SUM(" + KEY_INCORRECT + "), 0) FROM " + TABLE_NAME + " WHERE " + KEY_OBJECT + " = '" + _idObject + "') as 'Errores grupo' "
                    + " FROM " + TABLE_NAME;
            return dbcmd.ExecuteReader();
        }

        public IDataReader getDataFromResults(StudentEntity student, ObjectEntity objectEntity) {

            IDbCommand dbcmd = getDbCommand();
            dbcmd.CommandText =
                "SELECT"
                + "(SELECT COALESCE(SUM(" + KEY_CORRECT + "), 0) FROM " + TABLE_NAME + " WHERE " + KEY_STUDENT + " = " + student._id + " AND " + KEY_OBJECT + " = '" + objectEntity._id + "') as 'Aciertos usuario',"
                + "(SELECT COALESCE(SUM(" + KEY_INCORRECT + "), 0) FROM " + TABLE_NAME + " WHERE id_student = " + student._id + " AND id_object = '" + objectEntity._id + "') as 'Errores usuario',"
                + "(SELECT COALESCE(SUM(" + KEY_CORRECT + "), 0) FROM " + TABLE_NAME + " WHERE " + KEY_OBJECT + " = '" + objectEntity._id + "') as 'Aciertos grupo', "
                + "(SELECT COALESCE(SUM(" + KEY_INCORRECT + "), 0) FROM " + TABLE_NAME + " WHERE " + KEY_OBJECT + " = '" + objectEntity._id + "') as 'Errores grupo' "
                + " FROM " + TABLE_NAME
                + " LIMIT 1";
            return dbcmd.ExecuteReader();
        }
        ...
        public IDataReader getNumOfRows(StudentEntity student, ObjectEntity obj) {

            IDbCommand dbcmd = getDbCommand();
            dbcmd.CommandText =
                "SELECT COUNT(*) FROM " + TABLE_NAME + " WHERE " + KEY_OBJECT + " = '" + obj._id + "'";
            return dbcmd.ExecuteReader();
        }
    }
}