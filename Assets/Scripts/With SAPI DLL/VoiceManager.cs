...
public class VoiceManager : MonoBehaviour {
    ...
    */
    ...
    void Start() {
        ...
        if (VoicePreferred != "Microsoft Sabina Desktop") {
            for (int i = 0; i < VoiceNames.Length; i++) {
                if (VoiceNames[i] == "Microsoft Sabina Desktop") {
                    VoicePreferred = VoiceNames[i];
                    break;
                }
            }
        }
        ...
        if (VoicePreferred != "Microsoft Sabina Desktop") {
            for (int i = 0; i < VoiceNames.Length; i++) {
                if (VoiceNames[i] == "Microsoft Helena Desktop") {
                    VoicePreferred = VoiceNames[i];
                    break;
                }
            }
    ...
    /*
	public void Speak(string[] textToSpeak, Action onFinish = null, float delayEnd = 0f, float delayBetweenLines = 0f)
	{
		...
		string LangPreferredProperties = "<LANG LANGID=\"80A\">";
		//string VoicePreferredProperties = "<voice required='Name=" + VoicePreferred +"'>";
		//string VoicePreferredProperties = "<VOICE REQUIRED=\"language=10\">";
		textToSpeak = string.Format("{0}{1}", LangPreferredProperties, textToSpeak);

		Debug.Log("Text to Speak: " + textToSpeak);

		if(onFinish != null){
			FinishSpeaking(onFinish, delayEnd);
		}

		Uni_Voice_Speak(textToSpeak);
	}
	

	IEnumerator SpeakArray(List<string> listToSpeak, Action onFinish, float delayEnd, float delayBetweenLines){
		yield return new WaitForSeconds(0.1f);
		foreach(string textToSpeak in listToSpeak){
			string LangPreferredProperties = "<LANG LANGID=\"80A\">";
			Uni_Voice_Speak(string.Format("{0}{1}", LangPreferredProperties, textToSpeak));
			while(this.Status(0) == 2){
				yield return null;
			}
		}
	...
	*/
    ...
    public int Speak(string textToSpeak, Action speakNext, Action onFinish = null, float delayEnd = 0f) {
        // "This is <emph>very</emph> important!  This sounds normal <pitch middle = '+40'/> but the pitch drops half way through"
        // "This is <emph>very</emph> important!. This is very important! "
        // "<volume level="Level">  Text to be spoken</volume>"
        // "<rate speed="-5">This text should be spoken at rate zero.</rate>" -10 to +10
        ...
        FinishSpeaking(speakNext, onFinish, delayEnd);

        return Uni_Voice_Speak(textToSpeak);
    }

    public int Speak(string textToSpeak) {
        // "This is <emph>very</emph> important!  This sounds normal <pitch middle = '+40'/> but the pitch drops half way through"
        // "This is <emph>very</emph> important!. This is very important! "
        // "<volume level="Level">  Text to be spoken</volume>"
        // "<rate speed="-5">This text should be spoken at rate zero.</rate>" -10 to +10
    ...
    void FinishSpeaking(Action speakNext, Action onFinish, float delayEnd) {
        this.EnsureCoroutineStopped(ref typeRoutine);
        typeRoutine = StartCoroutine(FinishSpeakingCallback(speakNext, onFinish, delayEnd));
    }

    IEnumerator FinishSpeakingCallback(Action speakNext, Action onFinish, float delayEnd) {
        yield return new WaitForSeconds(0.1f);
        while (Status(0) == 2) {
            yield return null;
        }
